...
- Vite dev server 在 `vite.config.ts` 中显式绑定 `127.0.0.1`，与 `SpaProxyServerUrl` 保持相同的 IPv4 回环地址；不能只依赖 `localhost`，因为 macOS 上可能解析到 IPv6 `::1`，导致默认跳转到 `http://127.0.0.1:4173/` 时连接失败。
