...
- Vite dev server 在 `vite.config.ts` 中使用 `server.host=true` 监听所有网络接口，`SpaProxyServerUrl` 仍使用 `http://127.0.0.1:4173` 作为本机探测地址；这样既兼容 SpaProxy，也允许通过其他本机 IP 或局域网地址访问开发服务器。
